...
public class RedBlackTree<T extends Comparable<T>> implements MapGeneral<T, T> {
	...
	/**
	 * Returns sorted list of keys greater than key. Size of list will not exceed
	 * maxReturned
	 * 
	...
	private void getGreaterThan(RedBlackNode<T> node, Association key, List<T> list) {
		if (isNil(node)) {
			return;
